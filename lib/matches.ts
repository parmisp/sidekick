...
export function listMatches(viewer: UserRow): MatchSummary[] {
  const rows = db()
    .prepare(
...
export function getMatchForViewer(viewer: UserRow, matchId: string): MatchDetail | null {
  const row = db()
    .prepare(
    ...
    .get({ ...viewerParams(viewer), matchId }) as
    | { id: string; source: "swipe" | "comment"; created_at: number; other_id: string; is_seed: number; comment_text: string | null; comment_author: string | null }
    | undefined;
  if (!row) return null;
  return {
    id: row.id,
...
export function getMessages(matchId: string, after = 0): ChatMessage[] {
  return db()
    .prepare(
    ...
    .all(matchId, after) as ChatMessage[];
}
